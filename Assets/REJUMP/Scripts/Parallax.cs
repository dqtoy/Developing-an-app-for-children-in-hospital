...
public class Parallax : MonoBehaviour 
{
	...
	void Update () 
    {
        ...
        camOffset = (origin - camT.position).sqrMagnitude / 1000;

        //Change background layers texture offset based on camera offset and its scroll speed;
        for (int i = 0; i < layers.Length; i++)
            layers[i].background.material.mainTextureOffset = new Vector2(camOffset * layers[i].scrollSpeed / 200, 0);
...
public class Layers
{
    ...
    public float scrollSpeed = 1;          //Layer scroll speed;
